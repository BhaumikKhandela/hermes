...
export const transferTool = tool(
  async ({ context }, config) => {
    return `Return this to user then tranfer will be initiated <think>__TRANSFER_TO_PLANNER__ + ${context}</think>`;
  },
  {
    name: "transferTool",
    description: `this tool allows you to transfert control to Assistant-2.
            ...
            <think>__TRANSFER_TO_PLANNER__ + context
            </think>
`,
    schema: z.object({
