...
export function ImageEditorConfig({
  nodeId,
  credentialId,
  config,
  credentials,
  ...
  const [maskUrl, setMaskUrl] = useState<string>(config?.maskUrl || "");
  ...
  const handleSave = useCallback(() => {
    ...
    if (!isToolMode) {
      saveConfig.prompt = prompt;
      saveConfig.imageUrl = imageUrl;
      saveConfig.maskUrl = maskUrl;
    }
    save(saveConfig, selectedId);
  }, [save, prompt, imageUrl, maskUrl, selectedId, isToolMode]);
      ...
      {!isToolMode && (
        <section>
          ...
          <div className="rounded-xl border border-[#E7E7E7] bg-white divide-y divide-[#F0F0F0]">
            <div className="p-4">
              <label className="text-sm font-medium text-[#111827] mb-1.5 block">Source Image URL</label>
              <p className="text-xs text-[#6B7280] mb-2">URL of the image to edit.</p>
              <input
                type="text"
                value={imageUrl}
            ...
            <div className="p-4">
              <label className="text-sm font-medium text-[#111827] mb-1.5 block">Edit Prompt</label>
              <p className="text-xs text-[#6B7280] mb-2">Describe the edit to make.</p>
              <Textarea
                value={prompt}
                onChange={(e) => setPrompt(e.target.value)}
                placeholder="Add a pair of sunglasses to the subject..."
                rows={3}
            ...
            <div className="p-4">
              <label className="text-sm font-medium text-[#111827] mb-1.5 block">Mask Image URL (optional)</label>
              <p className="text-xs text-[#6B7280] mb-2">URL of a mask image. White areas will be edited, black areas preserved.</p>
              <input
                type="text"
                value={maskUrl}
