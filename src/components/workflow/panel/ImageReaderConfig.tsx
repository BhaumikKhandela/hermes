...
export function ImageReaderConfig({
  nodeId,
  credentialId,
  config,
  credentials,
  ...
  const [question, setQuestion] = useState<string>(config?.question || "");
  ...
  const handleSave = useCallback(() => {
    ...
    if (!isToolMode) {
      saveConfig.question = question;
      saveConfig.imageUrl = imageUrl;
    }
    save(saveConfig, selectedId);
  }, [save, question, imageUrl, selectedId, isToolMode]);

  const selectedCred = credentials.find((c) => c._id === selectedId);
  const providerIcon = resolveModelIcon("claude");
      ...
      <section>
        ...
        {!showCreateForm ? (
          ...
          <div className="mt-3 rounded-xl border border-[#E7E7E7] p-3 bg-white">
            <CredentialForm
              schema={getCredentialSchema("bluesmind" as any, "apiKey" as any)!}
      ...
      {!isToolMode && (
        <section>
          ...
          <div className="rounded-xl border border-[#E7E7E7] bg-white divide-y divide-[#F0F0F0]">
            <div className="p-4">
              <label className="text-sm font-medium text-[#111827] mb-1.5 block">Image URL</label>
              <p className="text-xs text-[#6B7280] mb-2">URL of the image to analyze.</p>
              <input
                type="text"
                value={imageUrl}
            ...
            <div className="p-4">
              <label className="text-sm font-medium text-[#111827] mb-1.5 block">Question (optional)</label>
              <p className="text-xs text-[#6B7280] mb-2">What would you like to know about this image?</p>
              <Textarea
                value={question}
                onChange={(e) => setQuestion(e.target.value)}
                placeholder="Describe this image in detail, including any text and objects..."
                rows={3}
